...
## Steps
1. Read `.design-system/accessibility/wcag-checklist.md` (POUR-organized, P0/P1/P2) and `.design-system/accessibility/aria-patterns.md`.
...
4. **Contrast — measure, don't eyeball.** For rendered HTML, RUN the real-render gates and report their actual output (AGENTS.md §9.2 Verification Protocol): `node .design-system/scripts/measure_render.mjs <file> [--dark]` (every text element) AND `node .design-system/scripts/verify_states.mjs <file> [--dark]` (every interactive element in default/hover/focus — catches hover-state failures). For loose color pairs, `python3 .design-system/scripts/contrast.py "<fg>" "<bg>"`. Never state a ratio you did not measure.
5. Check reduced-motion handling (`.design-system/taste/motion-choreography.md`).
