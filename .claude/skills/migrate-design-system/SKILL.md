...
## Steps
1. Read `.design-system/design-systems/interop-protocol.md` (Crosswalk Method, the three directions, headless-vs-styled guidance, verification).
2. Use the curated tables in `.design-system/design-systems/crosswalk.md` for Material 3 / Apple HIG / Fluent 2 / Carbon / shadcn/ui / Radix. For others, derive a mapping with the Crosswalk Method (map by role/intent across 6 axes: color roles, type scale, spacing unit, radius, elevation, motion).
...
4. **Verify** every mapped color pair for contrast (`.design-system/scripts/contrast.py` / `a11y-audit`); confirm all 8 states + dark mode survive the mapping.
