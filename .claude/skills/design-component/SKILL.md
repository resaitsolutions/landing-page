...
5. Apply visual judgment from `.design-system/taste/design-taste.md` (states, focus, no slop).
6. Optional fast start: `python3 .design-system/scripts/scaffold_component.py "<Name>"` to emit a stub, then fill it in.
...
- overlays/modals also: `node .design-system/scripts/verify_focustrap.mjs <harness> --open=<trigger>`
...
Build mobile-first; a fixed-px width that can't shrink is a bug. Run `node .design-system/scripts/verify_responsive.mjs <file|dir>` — it loads each harness at 280/320/414px and fails on any horizontal overflow. The four recurring causes and their fixes:
