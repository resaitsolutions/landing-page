...
- [ ] 1.1 Rewrite `content/0.index.yml` hero (`title`, `description`,
...
- [ ] 1.2 Rewrite the two `sections` feature blocks (currently
...
- [ ] 1.3 Rewrite the `features` grid (6 items) to describe
...
- [ ] 1.4 Rewrite the 9 `testimonials` items with generic learner
      ...
      identity claims).
- [ ] 1.5 Rewrite the `cta` block (title, description, links) to
      drive learner sign-up instead of "start building"/GitHub
      template links.

## 2. Pricing page rewrite

- [ ] 2.1 Rewrite `content/2.pricing.yml` `title`/`description`/`seo`
      for a learning-platform pricing page.
- [ ] 2.2 Rewrite the 3 plan tiers' `features` lists to learner-
...
- [ ] 2.3 Rewrite the `logos`/"Trusted by" section copy (or remove if
      it no longer fits — infra-provider logos like AWS/Heroku/
      Vercel/Cloudflare/Netlify don't belong on a learning-platform
      pricing page).
- [ ] 2.4 Rewrite the 6 FAQ items with real prospective-learner
...
- [ ] 3.1 Update `app/app.vue` `useSeoMeta` `titleTemplate` from
      `'%s - Nuxt SaaS template'` to Skilleate's own template.
- [ ] 3.2 Update `app/pages/index.vue` fallback `ogImage` URL
...
- [ ] 3.3 Update `AppFooter.vue`: remove/replace the GitHub link to
...
- [ ] 3.4 Review `AppHeader.vue` nav items (`Docs`, `Pricing`, `Blog`,
...
- [ ] 4.1 `.design-system/scripts/check_no_emoji.py` against every
      changed `content/*.yml` file and touched `.vue` files — zero
      emoji.
- [ ] 4.2 Read the full rewritten homepage and pricing copy end-to-end
      against `.design-system/content/voice-tone.md` for tone
      consistency (clear, concise, human) — not just spec compliance.
- [ ] 4.3 `pnpm dev`; browser-render `/` and confirm every spec
